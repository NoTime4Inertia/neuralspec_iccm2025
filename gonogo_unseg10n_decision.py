...
with model:
    ...
    hc_ppc = spa.State(assoc_ppc_vocab, neurons_per_dimension = 10)
    hc = spa.State(assoc_ppc_vocab, neurons_per_dimension = 10)
    ...
    ppc = spa.ThresholdingAssocMem(threshold=0.0, input_vocab=ppc_vocab, mapping = ppc_vocab.keys())

    
    # Feed forward
    stim >> obs
    spa.translate(hc, hc_vocab) >> hc_clean
    ...
    spa.translate(ppc, assoc_ppc_vocab) >> hc_ppc
    spa.translate(obs, assoc_ppc_vocab)*~spa.translate(hc_obs, assoc_ppc_vocab)*hc_ppc >> hc